  ...
  fetch('/api/intakes/'+id+'/provision',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'})
    .then(function(r){return r.json()}).then(function(d){
      if(d.ok){
        ...
        var em=d.email_queued?('<br><span style="color:#059669">✉️ Login details are being emailed to '+esc(d.sent_to)+'</span>')
              :(d.email_configured===false?'<br><span class="muted">✉️ Email not set up — copy the details and send them manually.</span>'
               :'<br><span class="muted">✉️ No email on file — copy the details and send them manually.</span>');
...
document.getElementById('createBtn').addEventListener('click',function(){
  ...
  fetch('/api/orgs/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
    .then(function(r){
      if(r.redirected||r.status===401){return {ok:false,error:'Session expired — refresh this page and log in again as the platform owner, then retry.'};}
      return r.text().then(function(t){
        try{return JSON.parse(t)}catch(e){return {ok:false,error:'Server error '+r.status+(t?(': '+t.replace(/<[^>]+>/g,' ').replace(/\\s+/g,' ').trim().slice(0,150)):'')};}
      });
    }).then(function(d){
      btn.disabled=false;
      if(!d.ok){toast(d.error||'Failed',1);return}
